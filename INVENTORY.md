...
**~$160.50/mo** — Phase 05 complete (graceful-shutdown closure + 3 chaos drills + 1 Helm/HPA SSA bug fix). Phase 04 baseline ~$160.50/mo unchanged. **Phase 05 added $0/mo of new infrastructure** — chart-level changes only (`preStop` + `terminationGracePeriodSeconds` config on existing Deployments, conditional `replicas:` rendering for SSA fix). The M3 drill terminated one EC2 instance and EKS provisioned a replacement automatically — same node group desired count, negligible incremental cost (~$0.05 for the ~few extra hours of dual-running during replacement). No new AWS resources of any kind. **ALB still scheduled for teardown after 2-week test horizon.** Domain `payservice.click` $3 one-time, auto-renew DISABLED.
...
2026-05-12 — Phase 05 closed. Added: `lifecycle.preStop.exec.command: ["sh", "-c", "sleep 15"]` and `terminationGracePeriodSeconds: 30` on both `payment-service` and `risk-check-service` Deployment templates (parameterized via `gracefulShutdown.preStopSleep` and `.terminationGracePeriodSeconds` in `values.yaml`). Chart fix: conditional `replicas:` rendering (`{{- if not .Values.hpa.enabled }}`) on both charts — fixes Helm + HPA Server-Side Apply field-ownership conflict surfaced during M4. Three chaos drills executed (M2 pod kill, M3 EC2 terminate, M4 image-pull failure) — all transient, no new permanent resources. `runbook.md` Incident Playbooks section populated with three playbooks (pod crashloop, node drained/lost, image pull failure). **Cumulative cost unchanged at ~$160.50/mo** — Phase 05 is essentially $0 new infrastructure; the only AWS-side activity was one EC2 instance terminated + replaced during M3 (~$0.05 of replication overhead).
