...
**2026-05-09 — M4b prep: removing `--reuse-values` (M2 fix) had a second side effect — the ALB ingress got torn down.**
Discovered while finding the ALB hostname for M4b's curl loop: `kubectl get ingress -A` returned nothing. The ALB went away after M2's deploy. Root cause: Phase 02 set `ingress.enabled=true` and `ingress.certificateArn=...` via `--set` at install time, and `--reuse-values` in CI (Phase 03) preserved those across every subsequent deploy. When we removed `--reuse-values` in M2 to fix the probes-block load, the next deploy used chart defaults — `ingress.enabled: false` — and Helm correctly tore the Ingress resource down. No CI failure because deletion isn't a failure.
...
- **Fix:** removed `--reuse-values` from `deploy-payment.yml`. Image overrides (repository, tag) are still passed via `--set` on every deploy, so the original intent (preserve image tag between deploys) is preserved.
- **Bonus consistency:** `deploy-risk-check.yml` never had `--reuse-values` — workflows had drifted. Removing it brings payment into alignment.
- **Lesson:** `--reuse-values` is a footgun for any future schema additions (new keys in values.yaml). The chart's `values.yaml` should be the source of truth for defaults, with `--set` only for runtime-injected values (image tag).
- **Atomic worked:** `--atomic` rolled the failed upgrade back automatically. Cluster state is unchanged from the prior release. Phase 03's safety net did its job.
